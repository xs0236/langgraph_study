...
# SUB_SUPERVISOR_PROMPT = """你是写作子图的调度主管。
...
# 调度规则：
# 1. 角度数为0 → parallel_research
# 2. 角度数≥3但无汇总笔记 → aggregator
# 3. 有调研笔记但无初稿 → writer
# 4. 有初稿但无编辑意见 → editor
# 5. 全部完成 → FINISH

# 只输出JSON：{{"next_agent": "...", "reasoning": "..."}}"""
...
# with SqliteSaver.from_conn_string("team_subgraph.db") as memory:
#     workflow = build_main_graph()
#     app = workflow.compile(checkpointer=memory)
    
#     config = {"configurable": {"thread_id": "team_subgraph_001"}}
#     inputs = {
#         "topic": "人工智能对现代教育的影响",
...
#             # 清空子图相关状态，保留 topic 和 previous_feedback
...
from langgraph.graph import StateGraph, END
from langgraph.graph.message import add_messages
from langgraph.checkpoint.sqlite import SqliteSaver  # ✅ 确认有这行！
from langgraph.types import Send
from langchain_openai import ChatOpenAI
from langchain_core.messages import HumanMessage, SystemMessage
