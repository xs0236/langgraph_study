...
如果总分低于7分或存在明显问题，should_rewrite 设为 true，并在 issues 中列出具体问题。""".format(article[:1000])
    ...
    def human_review(state):
        ...
        decision = interrupt({
            ...
            "message": "AI自检评分: {}。请输入操作：continue(继续) / stop(终止)，或输入具体修改意见让 AI 重写".format(
...
2. 每个平台应该采用什么风格？（如：微信-深度长文、知乎-专业分析、微博-轻松短句）
3. 哪些平台可能不太适合？为什么？""".format(article[:800])
    ...
    def m_s(s):
        ...
        # 发布
        platforms_str = s.get("ai_publish_platforms", "wechat,zhihu,weibo")
        platforms = [p.strip() for p in platforms_str.split(",") if p.strip()]
        need_pub = False
    ...
    for n in ["writing", "ai_quality_check", "human_review", "ai_image_decision", "image", "ai_publish_plan"]:
        mg.add_edge(n, "m_s")
    ...
    # ========== Gradio 界面 ==========
    with gr.Blocks(title="🏭 AI 内容工厂 - AI动态决策") as demo:
        gr.Markdown("""
        # 🏭 AI 内容工厂（AI 动态决策版）
        输入主题，自动完成：**写作 → AI自检 → 人工审核 → [Send并行]翻译 → AI决策配图 → 配图 → AI决策发布 → [Send并行]发布**

        > 🤖 新特性：
        > 1. **AI自检**：文章写完后 AI 自动评分，低于7分自动重写
        > 2. **AI配图决策**：AI 判断文章是否需要配图，不需要则跳过
        > 3. **AI发布计划**：AI 根据内容动态选择最适合的发布平台和风格
        > ⏪ 同时保留「时间旅行」功能
        """)
