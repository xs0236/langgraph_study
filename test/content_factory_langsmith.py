...
3. 哪些平台可能不太适合？为什么？""".format(article[:800])
    ...
    # ========== 串行翻译节点（替代原来的 Send 并行派发）==========
    def trans_fan_out(state):
        """串行执行翻译任务（普通节点必须返回 dict）"""
        print("  🌐 [翻译] 开始翻译...")
        updates = {}
        ...
        if not state.get("english_version"):
            ...
            updates["english_version"] = result
        
        if not state.get("japanese_version"):
            print("      🇯🇵 [翻译-日文] ...")
            prompt = "翻译成地道日文：\n\n{}".format(state["article"][:800])
            result = model.invoke([HumanMessage(content=prompt)]).content
            updates["japanese_version"] = result
        
        print("  🌐 [翻译] 完成")
        return updates

    # ========== 串行发布节点（替代原来的 Send 并行派发）==========
    def pub_fan_out(state):
        """串行执行发布任务（普通节点必须返回 dict）"""
        ...
        if "wechat" in platforms:
            print("      📱 [微信] ...")
            updates["wechat_url"] = "https://mp.weixin.qq.com/simulated/{}".format(hash(state["topic"]) % 100000)
        
        if "zhihu" in platforms:
            print("      💡 [知乎] ...")
            updates["zhihu_url"] = "https://zhuanlan.zhihu.com/p/simulated_{}".format(hash(state["topic"]) % 100000)
        
        if "weibo" in platforms:
            print("      🔥 [微博] ...")
            prompt = "生成微博文案（140字内）：\n{}".format(state["article"][:300])
            updates["weibo_text"] = model.invoke([HumanMessage(content=prompt)]).content
    ...
    def m_s(s):
        ...
        if features.get("publish"):
            ...
            if not s.get("wechat_url") and not s.get("zhihu_url") and not s.get("weibo_text"):
    ...
    def _build_final_yield(state, logs):
        ...
        if state.get("human_decision") == "stop":
            ...
            if state.get("ai_image_reason"):
                logs.append("🎨 配图决策: {}".format(state.get("ai_image_reason")))
            if state.get("ai_publish_platforms"):
                logs.append("📱 发布平台: {}".format(state.get("ai_publish_platforms")))
            if state.get("ai_publish_styles"):
                logs.append("🎨 风格建议: {}".format(state.get("ai_publish_styles")))
    ...
    with gr.Blocks(title="🏭 AI 内容工厂 - LangSmith监控") as demo:
        gr.Markdown("""
        # 🏭 AI 内容工厂（LangSmith 监控版）
        **已接入 LangSmith 全链路追踪！**

        > 📊 每次执行自动上报到 LangSmith，可查看：节点调用链、Token 消耗、延迟、输入输出、状态变化
        > 🔧 动态图构建 + AI 决策 + 时间旅行，功能全部保留
        """)
