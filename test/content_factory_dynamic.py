...
3. 哪些平台可能不太适合？为什么？""".format(article[:800])
    ...
    # ========== Gradio 界面 ==========
    with gr.Blocks(title="🏭 AI 内容工厂 - 动态图构建") as demo:
        gr.Markdown("""
        # 🏭 AI 内容工厂（动态图构建版）
        **运行时根据用户勾选的功能模块，动态构建执行图！**

        > 🔧 新特性：勾选需要的功能，AI 只执行你选中的模块，未选中的模块完全不会出现在执行图中。
        > 例如：只勾选「写作+翻译」，则不会执行配图和发布节点，流程更精简。
        """)
